...
Pose\'{\i}a una serie de \textit{renderers} que permiti\'{\i}an generar los cuestionario en los siguientes formatos:
\begin{itemize}
...
Para el renderer de HTML5 se le pod\'{\i}a pasar como argumento el path de una hoja de estilo para incorporarla al HTML de salida. Del mismo modo, se
pod\'{\i}a especificar el path de un template predefinido por el usuario de modo que las preguntas se renderizaran en el mismo.
...
\begin{itemize}
  ...
  alumno escrib\'{\i}a dicho \textit{distractor}, le apareciera la explicaci\'on de por qu\'e esa respuesta es incorrecta.
  
  [foto]
  
  \item {\bfseries Preguntas multirrespuesta con una  \'unica respuesta correcta}: las cl\'asicas preguntas tipo test. Se podía aleatorizar el orden
  ...
  \item {\bfseries Preguntas multirrespuesta con una  m\'ultiples respuestas correctas}: iguales a las multirrespuesta de opci\'on \'unica con la diferencia de 
...
XXXXX PONER EL CÓDIGO DE EJEMPLO QUE USABA ARMANDO Y COMO SE GENERA EL HTML5
...
Y las actividades a realizar en el mismo, tal cual est\'an descritas en la propuesta de Proyecto de Trabajo de Fin de Grado firmada por 
